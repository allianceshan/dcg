...
class DeepCoordinationGraphMAC(BasicMAC):
    ...
    def __init__(self, scheme, groups, args):
        ...
        self.utility_fun = self._mlp(self.args.rnn_hidden_dim, args.cg_utilities_hidden_dim, self.n_actions)  # 构建了一个从 hidden_dim到output 的网络结构 64-->5
        payoff_out = 2 * self.payoff_rank * self.n_actions if self.payoff_decomposition else self.n_actions ** 2 #25
        self.payoff_fun = self._mlp(2 * self.args.rnn_hidden_dim, args.cg_payoffs_hidden_dim, payoff_out)     # 128-->25
        ...
        self._set_edges(self._edge_list(args.cg_edges)) #[3,3,3,3]
    ...
    def payoffs(self, hidden_states):
        ...
        if self.payoff_decomposition: #no
            ...
            output = output.view(*(list(output.shape[:-1]) + [n, n])) #[2,1,6,5,5]
    ...
    def q_values(self, f_i, f_ij, actions):
        ...
        values = f_i.gather(dim=-1, index=actions).squeeze(dim=-1).mean(dim=-1) #一个数
        ...
        if len(self.edges_from) > 0:
            ...
            edge_actions = actions.gather(dim=-2, index=self.edges_from.view(1, -1, 1).expand(n_batches, -1, 1)) \
                * self.n_actions + actions.gather(dim=-2, index=self.edges_to.view(1, -1, 1).expand(n_batches, -1, 1))  #[1,6,1]
            values = values + f_ij.gather(dim=-1, index=edge_actions).squeeze(dim=-1).mean(dim=-1)   #一个数
    ...
    def greedy(self, f_i, f_ij, available_actions=None):
        ...
        best_value = in_f_i.new_empty(f_i.shape[0]).fill_(-float('inf'))  # bestQ 初始化为-inf
        best_actions = f_i.new_empty(best_value.shape[0], self.n_agents, 1, dtype=th.int64, device=f_i.device) #[1,4,1]
        ...
        if len(self.edges_from) > 0 and self.iterations > 0:
            messages = f_i.new_zeros(2, f_i.shape[0], len(self.edges_from), self.n_actions) #[2,1,6,5]
            for iteration in range(self.iterations):
                # Recompute messages: joint utility for each edge: "sender Q-value"-"message from receiver"+payoffs/E
                joint0 = (utils[:, self.edges_from] - messages[1]).unsqueeze(dim=-1) + f_ij                            #[1,6,5,5]
                joint1 = (utils[:, self.edges_to] - messages[0]).unsqueeze(dim=-1) + f_ij.transpose(dim0=-2, dim1=-1)   #[1,6,5,5]
                ...
                msg = torch_scatter.scatter_add(src=messages[0], index=self.edges_to, dim=1, dim_size=self.n_agents) #[1,4,5]
                ...
                if self.anytime:
                    ...
                    actions = utils.max(dim=-1, keepdim=True)[1] #[1,4,1]
                    value = self.q_values(in_f_i, in_f_ij, actions)  #调用算法2
    ...
    def _edge_list(self, arg):  # 默认是full
    ...
    def _set_edges(self, edge_list): # edge_list表示所有边的list，4个智能体是6条边。每个节点3条边
        ...
        for i, edge in enumerate(edge_list):
            self.edges_from[i] = edge[0]  # 边的起点
            self.edges_to[i] = edge[1]    # 边的终点
        ...
        self.edges_n_in = self.edges_n_in.float() # 以某节点为终点的节点个数 + 以某节点为起点的节点个数
