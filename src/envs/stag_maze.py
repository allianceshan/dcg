...
class StagMaze(MultiAgentEnv):
    ...
    def reset(self):
        ...
        # self._place_actors(self.prey[:self.n_stags, :, :], 1, row=[25,36], col=[0,9]) # 设置好目标位置

        # self._place_actors(self.agents, 0, row=self.mountain_agent_row if self.mountain_agent_row>= 0 else None ) # 随机设置智能体位置
        self._place_actors(self.agents, 0, row=[0,1], col=[0,4] ) # row=None
        # row=[np.random.randint(0,9) for i in range(4)]
        # col=[np.random.randint(0,9) for i in range(4)]
        # self._place_actors(self.agents, 0, row, col) # row=None

        # Place the stags/goats
        # self._place_actors(self.prey[:self.n_stags, :, :], 1, row=0 if self.mountain_spawn else None) # row=None
        self._place_actors(self.prey[:self.n_stags, :, :], 1, row=[5,9], col=[3,7]) # row=None
    ...
    def step(self, actions):
        ...
        for b in range(self.batch_size):
            for p in np.random.permutation(self.n_prey):
                if self.prey_alive[p, b] > 0:
                    ...
                    for u in range(4):
                        if not self._move_actor(self.prey[p, b, :], u, b, np.asarray([0, 1], dtype=int_type))[1]:
                            possible.append(u)
                    ...
                    if self.capture_action:  # False
                        ...
                        # Prey is caught when the number of possible moves is less or equal to their capture_condition 捕获条件，距离？
                        captured = len(possible) < self.capture_conditions[self.prey_type[p, b] - 1]#只要有智能体在周围就认为捕获！！！
                    # captured = captured and next_to_agent if self.args.prevent_cannibalism else captured
                    captured = next_to_agent
                    # If the prey is captured, remove it from the grid and terminate episode if specified
                    if captured:
                        # kill prey
                        self.prey_alive[p, b] = 0
                        self.grid[b, self.prey[p, b, 0], self.prey[p, b, 1], self.prey_type[p, b]] = 0 #不取消目标，即一直存在一直可以被检测到
                        # self.grid[b, self.prey[p, b, 0], self.prey[p, b, 1], 2] += self.delta_pos # 更新概率图 Q值
                        # terminate if capture_terminal=True
                        terminated[b] = terminated[b] or self.capture_terminal# False,所有猎物都被捕获才能终止
                        # determine reward for capture
                        rew = 0
                        # rew += self.capture_stag_reward if self.prey_alive[p, b] else 0  #只有第一次捕获目标有奖励
                        ...
                        if self.capture_freezes: #冻住捕获猎物的智能体
                            # each agent a...
                            for a in range(self.n_agents):
                                # ... which is not frozen...
                                if self.agents_not_frozen[a, b] > 0 \
        ...
        if self.batch_mode:
            return reward, terminated, info, self.steps
        else:
            return reward[0].item(), int(terminated[0]), info, self.steps
